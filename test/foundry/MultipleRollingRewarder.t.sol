...
// import "contracts/rewarders/ParentRewarderRolling.sol";
...
//     uint slope = 100; // Increase of multiplier every second
//     uint minMultiplier = 365 days * 100; // Arbitrary (but should be coherent with slope)
//     uint plateau = 10 days;
...
//         reliquary.addPool(
//             100, address(depositToken), address(0), linearCurve, "ETH Pool", address(0), true
//         );

//         uint pid = reliquary.poolLength() - 1;

//         parent = new ParentRewarderRolling(address(reliquary), pid);
...
//         rollingRewarderETH =
//             RollingRewarder(parent.createChild(address(rewardToken1), address(this)));
//         rollingRewarderUSDC =
//             RollingRewarder(parent.createChild(address(rewardToken2), address(this)));
...
//         uint relicId = reliquary.createRelicAndDeposit(address(this), 0, 100 ether);
...
//         uint ethReward = 1 ether;
//         uint usdcReward = 1000 * 10 ** 6;
...
//         uint earnedEth = IERC20(rewardToken1).balanceOf(address(this));
//         uint earnedUsdc = IERC20(rewardToken2).balanceOf(address(this));
...
//         uint rewardUser1 = IERC20(rewardToken1).balanceOf(user1);
//         uint rewardUser2 = IERC20(rewardToken1).balanceOf(user2);
...
//         uint rewardRelic1 = IERC20(rewardToken1).balanceOf(address(this));
//         assertApproxEqAbs(rewardRelic1, 10 ether + 5 ether, 1e5, "reward not expected");

//         reliquary.harvest(3, address(this));
//         uint rewardRelic2 = IERC20(rewardToken1).balanceOf(address(this)) - rewardRelic1;
...
//         uint rewardMergedRelic = IERC20(rewardToken1).balanceOf(user1);
//         assertApproxEqAbs(
//             rewardMergedRelic, 5 ether + (10 ether * uint(200) / 320), 1e5, "reward not expected"
//         );
//     }
